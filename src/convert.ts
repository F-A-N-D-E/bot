import { Valute } from "./type";

async function getValute (): Promise<string | Valute>{
   const url = "https://www.cbr-xml-daily.ru/daily_json.j";

    let result: Valute | string = await fetch(url)
...
export default async function convert (str: string){
    let arrProperti: string[] = str.toUpperCase().split(/\s+/g)
